...
list_of_length_of_names = [len(names) for names in list_of_names]
#print(list_of_length_of_names)

for name in list_of_names:
  print("The name {:} is {:} letters long".format(name, len(name)))

Roger = person(name=list_of_names[0],age=list_of_ages[0],height=list_of_heights_cm[0])
Mary  = person(name=list_of_names[1],age=list_of_ages[1],height=list_of_heights_cm[1])
Luisa = person(name=list_of_names[2],age=list_of_ages[2],height=list_of_heights_cm[2])
Elvis = person(name=list_of_names[3],age=list_of_ages[3],height=list_of_heights_cm[3])
...
plt.title('Height V.S. Age')
plt.xlabel('Age (in years)')
plt.ylabel('Height (in cm)')
plt.grid(True)
plt.savefig('heights_vs_age.png')
